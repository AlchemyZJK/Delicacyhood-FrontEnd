...
import { Content , Vedio } from '../exhibition/type';
...
export class VedioDetailComponent implements OnInit {
  ...
  remarkGetRes: RemarkResponse[] = []; // 获取评论响应包
    ...
    this.api.post('/api/1.0/poster/resource', {'postId': postId}).then(
      ress => {
        if (ress['status']) {
          ...
          this.proute = this.excontent2['source'];
      ...
      this.api.post('/api/1.0/collection/delete', this.collectionDeleteReq).then(
        res => {
          if (res['status']) {
            ...
            this.messageService.add({severity:'success', summary:'取消收藏成功', detail:'Delete Collection Success'});
          } else{
            console.log(res['message']);
