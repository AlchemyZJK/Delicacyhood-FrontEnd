...
import { Content , Menu , Data } from '../exhibition/type';
...
export class MenuDetailComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.route.params.subscribe(params => {
      ...
      this.mockService.getCont(this.postId).subscribe(content => this.content = content);
    ...
    this.api.post('/api/1.0/poster/resource', {'postId': postId}).then(
      ress => {
        if (ress['status']) {
          ...
          this.pauthorId = this.excontent2['authorId'];
          this.ptime = this.excontent2['createAt'];
          this.pcontent = this.excontent2['content'];
          for (const i of this.pcontent ) {
            if (i.imageFlag === true) {
              i.text = this.global.sourcePrefix + i.text; // zjk!!!
            }
          }
          this.pfruit = this.excontent2['fruit'];
          for (const i of this.pfruit) {
            if (i.imageFlag === true) {
              i.text = this.global.sourcePrefix + i.text;
            }
          }
          this.pintro = this.excontent2['intro'];
          for (const i of this.pintro) {
            if (i.imageFlag === true) {
              i.text = this.global.sourcePrefix + i.text;
            }
          }
          /*this.pictxt.push({title: this.ptitle, coverImg: this.pimg, postid: v['postId']});*/
          console.log(this.pcontent);
          this.get_authorName(this.pauthorId);
          /*for (const c of this.excontent) {
            console.log(c['title']);
          }*/
        }
      }
